// text2vol.cpp  version 3.4: Defines the entry point for the console application.
// doug fraker 2016-7, adding volume column, with values 70-7f
// and allow all notes, but removing pal support
// Z33 marks the changes in 3.3
...
unsigned int no_warnings = 0; // Z33 if != 0, silence warning about unsupported effects
...
void parse_song(int subsong, bool header_only)
{
	...
	for (pos = 0; pos<song_original.order_length; ++pos)
	{
		...
		shortest = song_original.pattern_length; //Z33 set full size, then shrink if D00/Bxx
		...
		for (chn = 0; chn<channels; ++chn)
		{
			for (row = 0; row<song_original.pattern_length; ++row)
			{
				...
				case 'D'://end pattern early
				{
					...
					{
						shortest = song_original.pattern[pos].length; //Z33
					}
				}
				break;
		...
		song_original.pattern[pos].length = shortest; //Z33
	}
}
			...
			for (row = 0; row<song_original.pattern[pos].length; ++row)
			{
				...
				if (chn<4)//pulse, triangle, and noise channels
				{
					if (ch->note<1 && ch->instrument >= 0) ch->instrument = -1;//ignore instrument numbers at empty rows and rest notes
																			   // ** changed from note < 2
					if ((pos == song_original.order_loop) && (insloop[chn] < 0)) //Z33
					{
						insloop[chn] = ins; //Z33
					}
					if (ch->instrument >= 0)
...
int output_header(char *songname, int song)
{
	...
	fprintf(outfile, ";this file for FamiTone3.4 library generated by text2vol tool\n");
...
int main(int argc, char *argv[])
{
	...
	for (i = 1; i<argc; ++i)
	{
		...
		if (!_stricmp(argv[i], "-Wno"))    no_warnings = 1; //Z33
